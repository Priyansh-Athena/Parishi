namespace GravityEngine2 {
    ...
    public class SGP4utils_GE2 {
         ...
         */
        public static bool TLEtoSatData(string tleData, ref SGP4SatData satrec)
        {
            // split TLE data into 3 lines
            string[] lines = tleData.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            if (lines.Length != 3) {
                // Debug.LogError("TLE does not have three lines");
                return false;
            }
            string satName = lines[0];
            string line1 = lines[1];
            string line2 = lines[2];

            SGP4unit.Gravconsttype whichconst = SGP4unit.Gravconsttype.wgs72;
            ...
            // satrec.line1 = line1;
            try {
                readLine1(line1, ref satrec);
            } catch (Exception e) {
                Debug.Log("Error Reading TLE line 1: " + e.ToString());
            ...
            // satrec.line2 = line2;
            try {
                readLine2(line2, ref satrec);
            } catch (Exception e) {
                Debug.Log("Error Reading TLE line 2: " + e.ToString());
                satrec.tleDataOk = false;
                satrec.error = 7;
                return false;
            }
            ...
            satrec.nddot = satrec.nddot * System.Math.Pow(10.0, satrec.nexp);
            satrec.bstar = satrec.bstar * System.Math.Pow(10.0, satrec.ibexp);
            ...
            satrec.jdsatepoch = JulianDate(year, mon, day, hr, minute, sec);
            char opsmodeImproved = 'i';
            // ---------------- initialize the orbit at sgp4epoch -------------------
            bool result = SGP4unit.sgp4init(whichconst, opsmodeImproved, satrec.satnum,
        ...
        public static string EpochDateString(ref SGP4SatData satData)
        {
            return string.Format("d/m/y {0}/{1}/{2} JDS={3}",
                satData.day, satData.month, satData.year, satData.jdsatepoch);
        }

        private static bool readLine1(string line1, ref SGP4SatData satrec)
        {
            string tleLine1 = line1; // first line
            if (!tleLine1.StartsWith("1 ")) {
                throw new System.Exception("TLE line 1 not valid first line");
            }

            // satnum
            satrec.satnum = (int)I18N.DoubleParse(tleLine1.Substring(2, (7 - 2)));
            // classification
            //satrec.classification = tleLine1.Substring(7, (8 - 7)); // 1 char
            // intln designator
            //satrec.intldesg = tleLine1.Substring(9, (17 - 9)); // should be within 8

            // epochyr
            satrec.epochyr = (int)I18N.DoubleParse(tleLine1.Substring(18, (20 - 18)));

            // epoch days
            satrec.epochdays = I18N.DoubleParse(tleLine1.Substring(20, (32 - 20)));

            // ndot
            satrec.ndot = I18N.DoubleParse(tleLine1.Substring(33, (43 - 33)));

            // nddot
            // nexp
            // div 1E5 since there is an implied decimal point
            if ((tleLine1.Substring(44, (52 - 44))).Equals("        ")) {
                satrec.nddot = 0;
                satrec.nexp = 0;
            } else {
                satrec.nddot = I18N.DoubleParse(tleLine1.Substring(44, (50 - 44))) / 1.0E5;
                //nexp
                satrec.nexp = (int)I18N.DoubleParse(tleLine1.Substring(50, (52 - 50)));
            }
            //bstar
            satrec.bstar = I18N.DoubleParse(tleLine1.Substring(53, (59 - 53))) / 1.0E5;
            //ibex
            satrec.ibexp = (int)I18N.DoubleParse(tleLine1.Substring(59, (61 - 59)));

            // these last things are not essential so just try to read them, and give a warning - but no error
            try {
                // num b.
                satrec.numb = (int)I18N.DoubleParse(tleLine1.Substring(62, (63 - 62)));

                //  elnum
                satrec.elnum = (long)I18N.DoubleParse(tleLine1.Substring(64, (68 - 64)));
            } catch (Exception e) {
                Debug.Log("Warning(ignorable): Error Reading numb or elnum from TLE line 1 sat#:" + satrec.satnum + " line1=" + line1
                    + "\n" + e.ToString());
            }

            // checksum
            //int checksum1 = (int) I18N.DoubleParse(tleLine1.Substring(68));

            // if no errors yet everything went ok
            return true;
        } // readLine1

        private static bool readLine2(string line2, ref SGP4SatData satrec)
        {
            ...
            // satnum
            int satnum = (int)I18N.DoubleParse(tleLine2.Substring(2, (7 - 2)));
            if (satnum != satrec.satnum) {
                Debug.Log("Warning TLE line 2 Sat Num doesn't match line1 for sat: "); // + satrec.name);
            }

            // inclination
            satrec.inclo = I18N.DoubleParse(tleLine2.Substring(8, (17 - 8)));

            // nodeo
            satrec.nodeo = I18N.DoubleParse(tleLine2.Substring(17, (26 - 17)));

            //satrec.ecco
            satrec.ecco = I18N.DoubleParse(tleLine2.Substring(26, (34 - 26))) / 1.0E7;

            // satrec.argpo
            satrec.argpo = I18N.DoubleParse(tleLine2.Substring(34, (43 - 34)));

            // satrec.mo (Mean anomoly deg)
            satrec.mo = I18N.DoubleParse(tleLine2.Substring(43, (52 - 43)));

            // no (Mean Motion rev/day)
            satrec.no = I18N.DoubleParse(tleLine2.Substring(52, (63 - 52)));

            // try to read other data
            try {
                // revnum
                satrec.revnum = (long)I18N.DoubleParse(tleLine2.Substring(63, (68 - 63)));
            } catch (Exception e) {
                Debug.Log("Warning: Error Reading revnum from TLE line 2 sat#:" + satrec.satnum + "\n" + e.ToString());
                satrec.revnum = -1;
            }

            //        // checksum
            //        int checksum2 = (int) I18N.DoubleParse(tleLine2.Substring(68));
                ...
                )
        {
            ...
            if (magh > small) {
                ...
                if (typeorbit.ToLower().Equals("ei") == true) // 0 = true for cpp strcmp
                {
                ...
                if (typeorbit.ToLower().Equals("ci") == true) {
                    arglat = angle(nbar, r);
                    if (r[2] < 0.0) {
                        arglat = twopi - arglat;
                    }
                ...
                if ((ecc > small) && (typeorbit.ToLower().Equals("ee") == true)) {
                    temp = ebar[0] / ecc;
                    if (Math.Abs(temp) > 1.0) {
                        temp = Math.Sign(temp);
                    }
                ...
                if ((magr > small) && (typeorbit.ToLower().Equals("ce") == true)) {
                    temp = r[0] / magr;
                    if (Math.Abs(temp) > 1.0) {
                        temp = Math.Sign(temp);
                    }
